...
void Test_CreationClass::addLine(const DL_LineData& data) {
    ...
    element data2 = { 1, data.x1, data.y1, data.x2, data.y2, 0 };
    this->list.emplace_back(data2);
}
...
void Test_CreationClass::addArc(const DL_ArcData& data) {
    ...
    element data2 = { 2, data.cx, data.cy, data.radius, data.angle1, data.angle2 };
    this->list.emplace_back(data2);
}
...
void Test_CreationClass::addCircle(const DL_CircleData& data) {
    ...
    element data2 = { 3, data.cx, data.cy, data.radius, 0, 0 };
    this->list.emplace_back(data2);
}
...
void Test_CreationClass::writeTxt() {
    std::ofstream out;
    out.open("out.txt");
    for (int i = 0; i < list.size(); ++i)
    {
        out << std::to_string(list[i].x1) << "," << std::to_string(list[i].x2) << "," << std::to_string(list[i].x3) << "," << std::to_string(list[i].x4) << "," << std::to_string(list[i].x5) << "," << std::to_string(list[i].x6) << std::endl;
    }
    out.close();
}
